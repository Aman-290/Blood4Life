...
$stmt = $conn->prepare("SELECT bloodgroup FROM patient WHERE email = ?");
if (!$stmt) {
...
echo "<h1>All donors with the blood group: " . htmlspecialchars($bloodgroup) . "</h1>";

$stmt2 = $conn->prepare("SELECT name, phone, email, bloodgroup, gender, birthdate, `weight`, state, zipcode, district, area, landmark FROM donor WHERE bloodgroup = ?");
if (!$stmt2) {
  die("Error: " . $conn->error);
}
